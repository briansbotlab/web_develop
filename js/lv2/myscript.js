...
var increasing_create_balls_change = 0;
...
function create_small_balls(){
    //亂數機率 
	var random_create_chance = Math.round(Math.random()*10-increasing_create_balls_change);
	
    if(random_create_chance < 2){
    var small_ball_tmp = new small_ball(120,60,15,enemy_ball_color,1,-1); //紫球
    }else if (random_create_chance >= 2){
    var small_ball_tmp = new small_ball(1000,360,15,friend_ball_color,1,-1); //黃球
    }
...
function show_miss_text(){
	...
	increasing_create_balls_change +=0.15;
    //sleep(300);
}
